...
- **שלב 3 (חלק ב') — Special Cells: Gold:** ✅ הושלם 2026-05-22 — סוג לוח חדש `'special_cells'` מאפשר תאים בלוח עם תכונות מיוחדות. פאזה זו ממשת רק את **תא זהב**: אריח שנוחת על תא זהב עולה דרגה אחת אוטומטית (capped ב-MAX_TIER). חידושים: `getSpecialCells()/setSpecialCells()/getSpecialCellAt(r,c)` ב-01-constants (מודל זהה למכפילים); `applyBoardToSession(board)` helper יחיד שמטפל גם ב-multipliers וגם ב-special_cells (refactor של per-mode init); CSS `.cell.special-gold` עם רינג זהוב נושם + ✨ פיניש (גלוי גם על תאים ריקים — השחקן יודע לכוון); באדמין: option `🔮 תאים מיוחדים` פעיל, עורך grid 4×6 קליקבילי, 4 פריסטים מהירים (שורה תחתונה / 4 פינות / צלב במרכז / נקה הכל) + ספירת תאים. סוגים נוספים (frozen/electric/locked/teleport/bonus) — תת-שלבים ג'-ה' עתידיים. המסגרת עומדת — הוספת סוג חדש תדרוש רק: (1) הרחבת `SPECIAL_CELL_TYPES` בשרת + (2) ענף חדש ב-drop/merge handler + (3) CSS class חדש.
- **שלב 3 (חלק ג') — Special Cells: Bonus:** ✅ הושלם 2026-05-22 — תא ירוק שמוסיף +N נקודות לכל מיזוג שהאריח השורד שלו נחת שם. amount מוגדר פר-לוח (50-10000) באדמין. מימוש: `SPECIAL_CELL_TYPES = ['gold','bonus']` בשרת + ולידציה ל-amount; קליינט preserves amount ב-`setSpecialCells`; engine ב-`processChains` בודק `getSpecialCellAt(kr,kc)` אחרי חישוב נקודות → מוסיף amount + badge ירוק "🪙 +N נקודות". באדמין: paint-mode selector חדש בראש העורך — בחר ✨ זהב או 🪙 בונוס, input סכום משותף לכל תאי הבונוס בלוח, ספירה מפצלת לפי סוג. אימוג'י 💎 הוחלף ב-🪙 כי 💎 כבר עומס יתר (קרדיטים + אריח דרגה 7) — וגרם לבלבול ״אני מקבל קרדיטים?״. ה-burst הפך לבועה אנימטיבית עם 🪙 מסתובב 720° + טקסט גדול + תווית ״נקודות״ קטנה כדי שאף אחד לא יחשוב שזה קרדיטים.
- **שלב 3 (חלק ד') — Special Cells: Frozen:** ✅ הושלם 2026-05-22 — תא קרח שהופך אריח שנוחת עליו לחסום: לא מתמזג ולא נופל עם הגרוויטציה. מנגנון puzzle אמיתי — שחקן צריך לעקוף או להסתמך על פצצה (event הקיים) כדי לפנות. שינויים אדריכליים גדולים יותר ממה שגולד או בונוס דרשו: (1) `findGroup` BFS מדלג על תאים שמסומנים `frozen` ב-`getSpecialCellAt` (גם כ-seed וגם כ-neighbor); (2) `applyGravity` רואה אריח על תא קפוא כ-anchor — הוא לא נופל, ו-write cursor קופץ ל-`row - 1` כדי שאריחים שנופלים מעליו יערמו עליו במקום לדרוס אותו. אריחים מתחת לאנקור נשארים ריקים (התא הקפוא חוסם את הגרוויטציה למטה). CSS: `.cell.special-frozen` ring כחול-לבן + ❄️ פיניש שמסתובב; כש-`.filled` — overlay קרח לבן+כחול ב-`::before` + `filter: saturate(0.55)` על ה-SVG כדי שהאריח ייראה "מועם" וקפוא. באדמין: paint mode שלישי "❄️ קפוא" + ספירה מפוצלת לשלוש ("✨ 3 · 🪙 2 · ❄️ 1"). אומת מתמטית: 3 בדיקות simulation על applyGravity עם anchors קפואים, כולן עברו. engine self-test עם 200 משחקים = 0 floating tiles.
- **שלב 3 (חלק ה') — Special Cells: Electric:** ✅ הושלם 2026-05-22 — תא צהוב חשמלי שמרחיב את ה-BFS של ה-merge group: כש-BFS חוצה תא חשמלי, הוא מוסיף 8 שכנים נוספים (רדיוס 2 + אלכסונים). אריחים מאותה דרגה ברחבי הלוח מתחברים למיזוג ענק. `SPECIAL_CELL_TYPES = ['gold','bonus','frozen','electric']`. `isElectricAt(r,c)` + הרחבת ה-stack ב-`findGroup` (8 שכנים נוספים כשהמיקום הנוכחי הוא electric). `triggerElectricFlash(group)` רץ אחרי כל merge שכולל תא electric: כל התאים בקבוצה מקבלים `.electric-flash` (yellow inset glow + brightness boost 1.7×), ומכל תא electric עפים 8 ⚡ emojis ב-8 כיוונים (n/ne/e/se/s/sw/w/nw — כל אחד עם keyframe נפרד). soundChain + buzz משולבים. ויזואל: `.cell.special-electric` ring צהוב חזק עם pulse 1.4s + ⚡ corner crackle 0.9s. באדמין: 4th paint mode ⚡ חשמלי, counter מפצל ל-4 ("✨ 2 · 🪙 3 · ❄️ 1 · ⚡ 2"), tooltip מבהיר "הכי מרהיב".
- **שלב 3 (חלק ו'+ז') — Special Cells: Locked + Teleport:** ✅ הושלם 2026-05-22 — סגירת כל 6 סוגי ה-special cells שתועדו במסמך המקורי. `🔒 נעול`: חוסם drop + gravity עד שמספר המיזוגים מגיע ל-N (1-30 בקביעת אדמין), ואז 🔓 נפתח עם burst דרמטי. אנקור בגרוויטציה (כמו קרח-עם-אריח), `unlock_after` נשמר פר-תא. `🌀 טלפורט`: אריח שנוחת קופץ מיד למיקום אקראי עם ספירלות סגולות בשני הקצוות. התא נשאר. שני הסוגים: paint modes, validation, helpers, animations, presets חדשים. **6/6 סוגי תאים הושלמו** — האדמין מקבל set מלא.
- **שלב 4 — Themed Boards (תמות חגים):** ✅ הושלם 2026-05-22 — שכבת theming ויזואלי שעוטה כל לוח (תאים + מכפילים) בפלטה של חג. שדה חדש `definition.theme_id` עם allowlist של 4 ערכות: 🕎 חנוכה / 💕 ולנטיין / 🇮🇱 יום העצמאות / 🍷 פסח. סוג חדש `'themed'` בתפריט האדמין שמשלב cells + multipliers + theme. הקליינט מוסיף `body.theme-{id}-active` class — CSS מטפלת בכל השאר: רקעים, glows על ה-app shell, אמוג'ים מרחפים (✡ ❤ 🌹 🍷 🍞), צבעי score-callout מותאמים. אפס JS — כל הדקורציות pure CSS. 4 presets חגיגיים במדריך — אפשר ליצור לוח חג קומפלט בקליק אחד (cells + theme + reshuffle mode + שם). תזמון: שלב עם `starts_at`/`ends_at` של תאריך החג והלוח יופיע/יעלם אוטומטית.
- **שלב 5 — Board Shapes (צורות לוח):** ✅ הושלם 2026-05-22 — שדה `definition.shape_id` חדש עם 4 צורות: ❤️ לב / 💎 יהלום / 🌲 עץ / 🔺 פירמידה. הלוח נשאר פיזית 4×6, אבל חלק מהתאים מקבלים `.shape-void` class — לא רנדורים, ה-engine מתעלם מהם (BFS/gravity/drop/isGameOver). `SHAPE_GEOMETRIES` ב-01-constants.js מכיל matrices 6×4 של 0/1. `isShapeInactiveAt(r,c)` משולב בכל מקום בו engine סורק את הלוח. 4 הפריסטים החגיגיים הקיימים שודרגו לכלול shape: ולנטיין=לב, יום העצמאות=יהלום, חנוכה=עץ, פסח=פירמידה. השילוב שלב 4 + שלב 5 = חוויית חג מלאה (רקע + ערכת צבעים + צורה ייחודית + תאים מיוחדים). cell count פר צורה: heart 15, diamond 16, tree 18, pyramid 20 (מתוך 24 = משחק קצר יותר ואסטרטגי יותר). **5/6 שלבי הroadmap הושלמו**.
- שלב 6 — LiveOps Scheduler: ⏳ עתיד. ראה הערות "✨ ניצול תשתית קיימת".
