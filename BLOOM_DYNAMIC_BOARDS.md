...
- **שלב 3 (חלק ג') — Special Cells: Bonus:** ✅ הושלם 2026-05-22 — תא ירוק שמוסיף +N נקודות לכל מיזוג שהאריח השורד שלו נחת שם. amount מוגדר פר-לוח (50-10000) באדמין. מימוש: `SPECIAL_CELL_TYPES = ['gold','bonus']` בשרת + ולידציה ל-amount; קליינט preserves amount ב-`setSpecialCells`; engine ב-`processChains` בודק `getSpecialCellAt(kr,kc)` אחרי חישוב נקודות → מוסיף amount + badge ירוק "💎 +N"; CSS `.cell.special-bonus` ring ירוק + 💎 פיניש (פלטה מ-`#9FE1CB`/`#4FBD8B`). באדמין: paint-mode selector חדש בראש העורך — בחר ✨ זהב או 💎 בונוס, input סכום משותף לכל תאי הבונוס בלוח, ספירה מפצלת לפי סוג ("✨ 3 · 💎 4"). הפריסטים (שורה תחתונה / פינות / צלב / נקה) ציובעים בצבע הנבחר עכשיו — מאפשר ליצור לוח ״פינות בונוס + שורה תחתונה זהב״ ב-3 קליקים.
- שלבים 3 (ד'-ה'), 4-6: ⏳ עתידיים. ראה הערות "✨ ניצול תשתית קיימת" בכל שלב.
