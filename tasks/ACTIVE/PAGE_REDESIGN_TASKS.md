...
| **UR.9** ✅ (`v20260606n`) | Per-tab hierarchy: `src/80-polish.js applyTabHierarchy()` spotlights the SINGLE genuinely-hot card per tab (claim / free spin / live deal / friends-invite, ranked via `TILE_PRIORITIES`, threshold 55) → full colour + warm lift while neighbours stay calm. The one-bright-card-amid-calm contrast = the per-tab anchor. Static, reduced-motion-safe; composes with the calm system (no filter collision). | Live-verified: rewards→free-spin, progress→league-reward, shop→starter-pack, social→friends-invite get the spotlight; passive tabs stay uniform. Screenshot `audit/15-tab-rewards-spotlight.png`. |
| **UR.4** ✅ (`v20260606m`) | Hero the live score in-game: the score cell now dominates the stats row (flex 1.7 + 30px, shrink classes bumped) and שיא/רצף/💎 demote to small chips — so the merge→`v2FlyParticlesToScore` stream lands on a target that visually matters (the core "one growing number" merge-game pull). CSS-only, `body.bloom-v2`-scoped. | Live-verified in-game: score 25–30px (flex 1.7) vs others 13px. Screenshot `audit/14-ingame-heroscore.png`. |
| **UR.8** ✅ (`v20260606o`) | Home fold: the hot-card → a thin one-line REWARD STRIP (181→64px, distinct from a tab card) and the 4 play-mode actions (תחרות/דו-קרב/אתגרים/סקינים) → a compact 4-col row reordered ON the fold (above pid; identity demoted to the footer). The highest-retention loops are now visible on first paint. `src/31-home-variants.js` + `home-v2.css`, tiles-scoped. | Live-verified @390×844: hot-strip 64px, actions at y=549 (on the 844 fold), pid below. Screenshot `audit/16-home-fold.png`. |
| **UR.10** ✅ (`v20260606p`) | Game-over: collapse the heavy analytics (top-50 leaderboard + scoring table + per-tier game-stats + emoji share-card, ~250px) behind a single `<details>` ("📊 הפירוט המלא + שיתוף") so the end-screen is a DECISION (score → celebration → שחק שוב → 📤 share-win → return hook), not a report. `src/12-tour-info.js` + `screens.css`. | **Played a real game to a 58K/186K crown game-over via Playwright** and verified: details collapsed by default, leaderboard/tier-table/game-stats all moved inside, play-again + share + home all reachable. Screenshots `audit/17-gameover-real.png` (before, 20 stacked blocks) → `audit/18-gameover-collapsed.png` (after). |
...
|---|---|---|---|---|---|---|---|---|---|
| **In-game (v2 board)** | 80 | 70 | 70 | 74 | 70 | 82 | 66 | **63 → ~76** ✅ | Clean board + the live score now HEROES the stats row (merge particles land on a worthy target). Remaining: col-mult intent (UR.2, admin), orphaned "+52". |
| **Home (tiles)** | 90 | 86 | 84 | 86 | 90 | — | 82 | **77 → ~90** ✅ | Hot-card → slim reward strip (181→64px); תחרות/דו-קרב/אתגרים/סקינים now ON the fold; pid demoted to footer; tier-maxed score chase; ladder shows ONE "you are here" marker (not 8 ✓). The fold funnels to PLAY + the competitive loops. |
| **Tab pages (×4)** | 78 | 74 | 74 | 76 | 80 | 82 | 64 | **60 → ~80** ✅ | Calm unified cards + per-tab SPOTLIGHT (one hot card vivid, rest calm) = clear anchor per tab. Remaining: de-dup (UR.8), stack hot card to top. |
| **Game-over** | 84 | 72 | 74 | 72 | 76 | 80 | 66 | **69 → ~78** ✅ | Verified on a REAL overloaded end-screen (186K/crown/new-best, 20 blocks). The leaderboard + scoring table + game-stats + emoji-share now collapse behind one tap, so it reads as a DECISION (score → celebration → שחק שוב → share → return hook). Remaining: cap the mid banners (claim-name/rival/continue/ad) to 2. |
