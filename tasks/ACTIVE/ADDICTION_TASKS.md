...
## ✅ session 4 batch 6 (2026-06-02) — הזרעת טורנירים בבוטים (#5/#12) [server-only]
> בלי שינוי client → בלי cache-bump. נפרס server.
...
> **✅ החלטות בעלים התקבלו (2026-06-02) — לבנות שניהם DEFAULT-OFF + שליטת-אדמין מלאה + ? tooltips. נדחה לסשן רענן (קוד-הליבה הכי רגיש, לא לסכן בעייפות-context אחרי 16 פריסות):**
> - **#43 energy ל-core loop** — הבעלים: "תעשה הכי טוב שיגרום לשחקן להרגיש מכור, והכי חשוב שהאדמין תמיד יכול לשנות." ⇒ לבנות: הרחב Lives ל-daily/practice מאחורי `lives_apply_to_core` (**DEFAULT 'false'** — ברירת-מחדל לא נוגעת בלולאה הראשית = אפס סיכון), שער נדיב (8 חיים, 1/15דק), `ensureLifeForCoreGame()` שמחזיר true מיד כשכבוי, hook בכל נקודות-הכניסה ל-daily/practice (05a-home-v2: ה-CTA הראשי + 255/725/820), admin TIP+PRESET. **engine self-test אחרי כל שלב.**
> - **#44 drop-arc** — הבעלים: "תעשה מה שנראה לך, ובאדמין אסביר מה זה עושה ואיך לשנות עם ? ללחיצה." ⇒ לבנות: אריחים חדשים ב-drop-container נפרד (`position:fixed`, translateY(-100%), 0.4-0.6s ease-out) שלא נוגע ב-render/innerHTML, **`drop_animation_enabled` DEFAULT 'false'** + `drop_duration_ms`, admin TIP מפורט עם ? (מה זה + איך לכוונן). **engine self-test (200 משחקים / 0 floating) אחרי כל שלב; rollback אם מסתבך.**
> - **#27 FTUE** — הבעלים לא בטוח; המלצתי להשאיר את ה-demo המתוסרט (ROI נמוך מול סיכון-engine). מוחזק אלא אם הבעלים יאשר במפורש re-arch.
