...
## ✅ בוצע ונפרס לייב (2026-05-31)
> **כל 8 הבאגים הקריטיים (AD.1)** + **משימה מדורגת #1 — צי-בוטים אוטומטי (AD.2/AD.2.1)** + **AD.2.2 (תיקון אמינות סטטיסטיקה)** הושלמו, נפרסו ואומתו בשרת החי. פירוט מלא ב-CLAUDE.md (טבלת "Audit fixes shipped").
> - 🟢 **העולם חי אוטומטית**: `activeNow` עלה מ-0 ל-6 בלי התערבות. בוטים נדלקים ב-boot, שליטה מלאה מהאדמין (`bots_auto_*` + כפתור "🌍 מלא עולם").
> - 🟢 **"הלוח של היום" תוקן**: מצביע כעת על לוח אמיתי (זהב #2) במקום לוח ריק #3.
> - 🟢 באגים נוספים: ROLLBACK במלחמת גילדות, error-middleware, toast מעל ה-nav, מד חיים, תשלום-הימור אמיתי במרוץ-חי + כפתור פרישה, fallback שיתוף-תחרות, week≥today.
> - ℹ️ #18 (פעימת כפתור שיתוף) — היה false-positive: כבר קיים (`replayShareBtnPulse`).
>
> **נותר לביצוע (ספרינטים 3-6, מומלץ session חדש)**: #2/#36 באנר מכפיל תוך-משחק, #3 ספירת-פרס-הבא, #4 הפעלת push, #5-6 הזרעת לוחות+טורניר אוטומטי, #7 push loss-aversion, #8 חגיגת חזרה, #13 מד-danger, #15 מודרציית-שחקן, #14 עורכי spin/trophy, #16 כלכלה faucet-vs-sink, #19-20 חברתי/גילדה, #21 שם חיית-מחמד, #22/#37 design tokens, ועוד.
