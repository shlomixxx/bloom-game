...
### - [x] **TB.3** — Audit לבעיות תצוגה נוספות ✅
> בדוק שכל אלמנט שנוסף ב-49 stages לא לוקח גובה מהלוח.
>
> **בוצע (2026-05-25)**: סריקה של `grep -rn "insertBefore.*grid-wrap\|parentNode.insertBefore" src/*.js` מצאה רק 2 hits: `col-mult-bar` (כבר טופל ב-TB.2) ו-modal של shop (מוזרק לתוך grid-wrap כ-overlay, לא לוקח גובה in-flow). מבנה ה-`.app` נקי: `.top` → `#mode-bar` → `#tier-bar` → `#grid-wrap`. ה-tier-bar הוא game UI לגיטימי. הוספות בעתיד שיכניסו אלמנטים בין mode-bar ל-grid-wrap צריכות להיכנס כ-floating bar (כמו TB.1) או overlay בתוך grid-wrap.
...
### - [x] **TD.1** — Tomorrow Preview ב-Streak Display ✅
> שחקן יראה "מחר: +200💎" → loss aversion.
>
> **בוצע (2026-05-25)**: `src/05a-home-v2.js` `renderHeroBannerV2` הוסיף `tomorrowPreviewHtml()` helper שמחשב את ה-reward של מחר מול היום מ-`getDailyRewardAmount(streakDay)` (25→50→100→200 לפי 1/3/7/30 ימים). כשהפרס של מחר *גדול* מהיום (לדוגמה streak=2 → היום 25💎, מחר 50💎), מציג pill gold-pink pulsing "מחר: +50💎 (במקום +25)" — escalation visible. כשאין שדרוג (streak=30+), מציג pill נייטרלית "מחר: +200💎" כתזכורת לטוב. CSS חדש ב-`public/css/home-v2.css` עם dark theme overrides + `heroTomorrowPulse` animation. ההצגה רק ב-streak hero (יום 3+ ויום 7+) כך שלא מטריפה שחקנים מתחילים.
...
### - [x] **TD.2** — Ghost Replay Push Notification ✅
> חבר ניצח אותך → push → אתה רוצה לחזור.
>
> **בוצע (2026-05-25)**: `server.js` helper חדש `_notifyFriendsBeatenOnDaily(senderDeviceId, senderScore, senderName, date)` מותחבר ל-`POST /api/score` (daily). הוא מוצא חברים (sym friendships table) שהציון היומי שלהם **קטן ב-500+** מציון השולח באותו תאריך, ומפעיל `sendPushToDevice` עם title "שם עבר אותך! 👑" + body עם הציון + URL `/?mode=daily` כדי לנחות ישר ב-daily. dedup לפי `_ghost_push:<sender>:<recipient>:<date>` כך ש-improving הציון לא שולח 5 pushes (אחד ביום מקסימום). שני config keys: `ghost_push_enabled='true'` + `ghost_push_min_lead='500'`. ה-helper נקרא fire-and-forget כך שלא חוסם את ה-/api/score. השילוב עם A9 Ghost Mode הקיים: השחקן שמקבל push יכול לרוץ נגד ה-ghost של החבר ב-`/?mode=daily`.
...
### - [x] **TD.3** — Streak Freeze Push ✅
> יום בלי משחק = push "הקפא הרצף שלך! 200💎".
>
> **בוצע (2026-05-25)**: שדרוג ה-streak_danger signal הקיים ב-`_pickSmartPushFor` (smart-push scheduler ב-server.js, רץ כל 30 דק' בשעות 9-22 שעון ישראל). כשהמערכת מזהה שחקן ששיחק אתמול אבל לא היום, היא בודקת את היתרה שלו: אם יש לו ≥200💎 (`dyn_streak_freeze_price`) והאופציה enabled, ה-push משתנה ל-"🛡 הקפא את הרצף שלך · יש לך 200💎? הקפא את הרצף ל-24 שעות נוספות" במקום הגנרי "שמור על הרצף שלך". מציע את החלופה הדפנסיבית במקום רק להזהיר. `_loadSmartPushConfig` עכשיו טוען גם `dyn_streak_freeze_*` keys (LIKE כפול בשאילתה אחת). הסיגנל החדש מקבל reason 'streak_freeze_offer' למעקב נפרד באדמין.
...
| **B — Display Size** | 3 | 3 | 🟢 הושלם |
| **C — Stuck Screens** | 6 | 6 | 🟢 הושלם |
| **D — Addiction Boosters** | 3 | 3 | 🟢 הושלם |
| **סה"כ** | **16** | **16** | 🟢🟢🟢 |
